...
function Install()
{
  ...
  URLKernel='https://github.com/BwmBama/lotServer/raw/master/lotServer.log'
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://raw.githubusercontent.com/BwmBama/lotServer/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://raw.githubusercontent.com/BwmBama/lotServer/master/lotServer.tar"
